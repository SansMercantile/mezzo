    ...
    <nav style={{ position: "sticky", top: 0, zIndex: 100, display: "flex", alignItems: "center", justifyContent: "space-between", padding: "0 36px", height: 64, background: "rgba(253,252,248,0.97)", borderBottom: "1px solid #C9A84C18", backdropFilter: "blur(10px)" }}>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 10, flexShrink: 0 }}>
        {!isLoading && (isAuthenticated ? (
          <>
            {user?.picture && <img src={user.picture} alt={user.name || "Profile"} style={{ width: 26, height: 26, borderRadius: "50%" }} />}
            <button onClick={() => logout({ logoutParams: { returnTo: window.location.origin } })} style={{ padding: "8px 14px", background: "none", color: INK, fontFamily: "'Cinzel',serif", fontSize: 8, letterSpacing: "0.2em", textTransform: "uppercase", border: `1px solid ${G}`, cursor: "pointer" }}>
              Log Out
            </button>
          </>
        ) : (
          <button onClick={() => loginWithRedirect()} style={{ padding: "8px 14px", background: "none", color: INK, fontFamily: "'Cinzel',serif", fontSize: 8, letterSpacing: "0.2em", textTransform: "uppercase", border: `1px solid ${G}`, cursor: "pointer" }}>
            Log In
          </button>
        ))}
        <button onClick={() => nav("dashboard")} style={{ padding: "8px 18px", background: G, color: "#FFF8DC", fontFamily: "'Cinzel',serif", fontSize: 8, letterSpacing: "0.2em", textTransform: "uppercase", border: "none", cursor: "pointer" }}>
          Dashboard
        </button>
      </div>
    </nav>
